...
export const updateExtensionPaymentScheduleProcedure = protectedProcedure
  .input(inputSchema)
    ...
    const results = await Promise.all(
      subscriptions.map(async (sub) => {
        try {
          ...
          const subscription =
            await ctx.db.query.extension_subscriptions.findFirst({
              where: (extension_subscriptions, { eq }) =>
                eq(extension_subscriptions.id, sub.id)
            })
